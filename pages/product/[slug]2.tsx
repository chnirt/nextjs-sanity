import { definedProductQuery, productQuery } from '@lib/query'
...
export default function ProductScreen({ data, preview }: ProductProps) {
  ...
  const price = numberWithCommas(
    product?.price + selectedColor?.price + product?.storages?.[0]?.price
  )

  console.log(product?.price, product?.colors?.[0], product?.storages?.[0])
...
export async function getStaticProps({ params, preview = false }: any) {
  const productResponse = await getClient(preview).fetch(productQuery, {
    slug: params.slug,
  })
  ...
  const product = {
    ...
    price: _.get(productResponse, ['price']) ?? 0,
...
export async function getStaticPaths() {
  const paths = await getClient().fetch(definedProductQuery)
